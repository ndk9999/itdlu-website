...
namespace DLUProjectMvc.Areas.Admin.Controllers
{
    ...
    {
        private IServices<Category> _service;
        private CategoryExtension _cateExt;
      
        public CategoryController(IServices<Category> service, CategoryExtension cateExt         )
        {
        ...
        {
            int rs = _service.Delete(new Category { CategoryID = id });
            if (rs > 0)
            {
