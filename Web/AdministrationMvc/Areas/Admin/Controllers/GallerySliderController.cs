...
namespace DLUProjectMvc.Areas.Admin.Controllers
{
    ...
    public class GallerySliderController : BaseController
    {
        private IServices<GallerySlider> _service ;
        ...
        {
            return View(new GallerySlider {GalleryID = -1, DateCreated=DateTime.Now, ImageUrl="/Content/images/noimage.png", IsPublished=true });
        }
        
        [HttpPost]
        [ValidateAntiForgeryToken]
        [ValidateInput(false)]
        public ActionResult Create(GallerySlider model)
        {
        ...
        [HttpPost]
        public ActionResult Edit(GallerySlider model)
        {
             if (ModelState.IsValid)
            {
        ...
        {
            int rs = _service.Delete(new GallerySlider { GalleryID = id });
            if (rs > 0)
            {
