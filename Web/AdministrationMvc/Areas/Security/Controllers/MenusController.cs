...
namespace DLUProjectMvc.Areas.Security.Controllers
{
    ...
    {
        private IServices<Menus> _service;
        private MenusExtension _menuExt;
        private IServices<Pages> _pageService;
        private IServices<Category> _categoryService;
        private IServices<NoticeCategory> _noticeCategoryService;
        public MenusController(IServices<Menus> service, MenusExtension menuExt,
            IServices<Pages> pageService, IServices<Category> categoryService, IServices<NoticeCategory> noticeCategoryService)
        {
            this._service = service;
            this._menuExt = menuExt;
            this._pageService = pageService;
            this._categoryService = categoryService;
            this._noticeCategoryService = noticeCategoryService;
        }
        public ActionResult Index(int? page, int? pageSize, string DisplayFlag, string queryString)
        {
            ...
            //{
            //    var display = ConvertType.ToEnum<DisplayFlagMenuEnum>(DisplayFlag);
            ...
            //}
            var model = _menuExt.GetAllMenus(pageIndex, pageSize1, DisplayFlag, queryString, false);
            InitData();
        ...
        void InitData()
        {
            ViewBag.ParentMenus = _menuExt.GetAllDropdownList(false);
        }
        ...
        [ValidateInput(false)]
        public ActionResult Create(Menus model)
        {
        ...
        [ValidateInput(false)]
        [HttpPost]
        public ActionResult Edit(Menus model)
        {
            if (ModelState.IsValid)
            {
        ...
        {
            int rs = _service.Delete(new Menus { MenuID = id });
            if (rs > 0)
            {
        ...
        {
            var model = new MenuPopupModel
            {
                ListCategory = _categoryService.All().Where(c => c.IsPublished == true).ToList(),
                ListPage = _pageService.All().Where(c => c.IsPublished == true).ToList(),
                ListNoticeCategory = _noticeCategoryService.All().Where(c => c.IsPublished == true).ToList()
            };
            return View(model);
        }
    }
