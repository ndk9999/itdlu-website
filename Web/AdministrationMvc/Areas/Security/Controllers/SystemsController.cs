/* FileName: SystemsController.cs
Project Name: DLUProject
Date Created: 6/11/2014 10:54:03 PM
...
namespace DLUProjectMvc.Areas.Security.Controllers
{
    ...
    {
        private IServices<Systems> _service ;
        ...
        {
            return View(new Systems { SystemID = -1, Image = "/content/images/noimage.png" });
        }
        
        [HttpPost]
        [ValidateAntiForgeryToken]
        [ValidateInput(false)]
        public ActionResult Create(Systems entity)
        {
			if (ModelState.IsValid)
            {
                var rs = _service.Insert(entity);
                if (rs > 0)
                {
        ...
        [HttpPost]
        public ActionResult Edit(Systems model)
        {
             if (ModelState.IsValid)
            {
        ...
        {
            int rs = _service.Delete(new Systems { SystemID = id });
            if (rs > 0)
            {
