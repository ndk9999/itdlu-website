/* FileName: FunctionController.cs
Project Name: DLUProject
Date Created: 7/11/2014 12:04:44 AM
...
namespace DLUProjectMvc.Areas.Security.Controllers
{
    ...
    {
        private IServices<Function> _service ;
        WorkGroupExtension _workGroupExt;
        public FunctionController(IServices<Function> service, WorkGroupExtension workGroupExt)
        {
            this._service = service;
            this._workGroupExt = workGroupExt;
        }
        public ActionResult Index(int? page, int? pageSize, string WorkGroupID, string queryString)
        {
            ...
            int wId = -1;
            if (!string.IsNullOrEmpty(WorkGroupID)) wId = WorkGroupID.ToInt();
            if (wId > 0)
            {
                myList = myList.Where(c => c.WorkGroupID == wId).ToList();
            }
            ...
            if (wId > 0 && !String.IsNullOrEmpty(queryString))
            {
                queryString = queryString.ToLower();
                myList = myList.Where(c => c.WorkGroupID == wId && string.Format("{0} {1}", c.Name, c.Url).ToLower().Contains(queryString)).ToList();
            }
        ...
        void InitData()
        {
            ViewBag.WorkGroups = _workGroupExt.GetAllDropdownList(true);
        }
        public ActionResult Create()
        {
            InitData();
            return View(new Function { FunctionID = -1, Image = "/content/images/noimage.png" });
        }
        
        [HttpPost]
        [ValidateAntiForgeryToken]
        [ValidateInput(false)]
        public ActionResult Create(Function model)
        {
        ...
        {
            int rs = _service.Delete(new Function { FunctionID = id });
            if (rs > 0)
            {
