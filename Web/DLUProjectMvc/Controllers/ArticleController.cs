    ...
    public class ArticleController : Controller
    {
       ...
       [Route("tin-tuc")]
        public ActionResult Index()
        {
            ...
            {
                ContentSliders = _contentService.All().Where(c => c.IsPublished == true && c.IsDeleted == false).Take(5).ToList(),
                Categories = _categoryService.All().Where(c=>c.IsPublished==true).ToList(),
                ListContent=_contentService.All().Where(c=>c.IsPublished==true &&c.IsDeleted==false).ToList()
            };
            return View(model);
        }
        ...
        public ActionResult Single(int id)
        {
            ...
            {
                Item = model,
                LatestContent = _contentService.All().Where(c => !c.ContentID.Equals(id) && c.IsPublished == true && c.IsDeleted == false).Take(4).ToList(),
