...
namespace DLUProject.API.Controllers
{
    ...
    {
        public IServices<Unit> _service { get; set; }
        public UnitController(IServices<Unit> service)
        {
            this._service = service;
        }
        // GET api/account
        public JsonResponse<Unit> Get()
        {
            var items = _service.All();
            return new JsonResponse<Unit> { Success = true, ListData = items };
        }

        // GET api/Unit/5
        public JsonResponse<Unit> Get(int id)
        {
            var model = _service.Get(id);
            return new JsonResponse<Unit> { Success = model != null, Data = model };
        }

        // POST api/Unit
        public JsonResponse Post([FromBody]Unit value)
        {           
        ...
        // PUT api/Unit/5
        public JsonResponse Put([FromBody]Unit value)
        {           
