/* FileName: NoticeCategoryController.cs
...
{
    public class NoticeCategoryController : ApiController
    {
        public IServices<NoticeCategory> _service { get; set; }
        public NoticeCategoryController(IServices<NoticeCategory> service)
        {
            this._service = service;
        }
        // GET api/account
        public JsonResponse<NoticeCategory> Get()
        {
            var items = _service.All();
            return new JsonResponse<NoticeCategory> { Success = true, ListData = items };
        }

        // GET api/NoticeCategory/5
        public JsonResponse<NoticeCategory> Get(int id)
        {
            var model = _service.Get(id);
            return new JsonResponse<NoticeCategory> { Success = model != null, Data = model };
        }

        // POST api/NoticeCategory
        public JsonResponse Post([FromBody]NoticeCategory value)
        {           
        ...
        // PUT api/NoticeCategory/5
        public JsonResponse Put([FromBody]NoticeCategory value)
        {           
