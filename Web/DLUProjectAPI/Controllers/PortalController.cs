...
namespace DLUProject.API.Controllers
{
    ...
    {
        public IServices<Portal> _service { get; set; }
        public PortalController(IServices<Portal> service)
        {
            this._service = service;
        }
        // GET api/account
        public JsonResponse<Portal> Get()
        {
            var items = _service.All();
            return new JsonResponse<Portal> { Success = true, ListData = items };
        }

        // GET api/Portal/5
        public JsonResponse<Portal> Get(int id)
        {
            var model = _service.Get(id);
            return new JsonResponse<Portal> { Success = model != null, Data = model };
        }

        // POST api/Portal
        public JsonResponse Post([FromBody]Portal value)
        {           
        ...
        // PUT api/Portal/5
        public JsonResponse Put([FromBody]Portal value)
        {           
