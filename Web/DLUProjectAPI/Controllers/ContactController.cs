...
namespace DLUProject.API.Controllers
{
    ...
    {
        public IServices<Contact> _service { get; set; }
        public ContactController(IServices<Contact> service)
        {
            this._service = service;
        }
        // GET api/account
        public JsonResponse<Contact> Get()
        {
            var items = _service.All();
            return new JsonResponse<Contact> { Success = true, ListData = items };
        }

        // GET api/Contact/5
        public JsonResponse<Contact> Get(int id)
        {
            var model = _service.Get(id);
            return new JsonResponse<Contact> { Success = model != null, Data = model };
        }

        // POST api/Contact
        public JsonResponse Post([FromBody]Contact value)
        {           
        ...
        // PUT api/Contact/5
        public JsonResponse Put([FromBody]Contact value)
        {           
