...
namespace DLUProject.API.Controllers
{
    ...
    {
        public IServices<Photo> _service { get; set; }
        public PhotoController(IServices<Photo> service)
        {
            this._service = service;
        }
        // GET api/account
        public JsonResponse<Photo> Get()
        {
            var items = _service.All();
            return new JsonResponse<Photo> { Success = true, ListData = items };
        }

        // GET api/Photo/5
        public JsonResponse<Photo> Get(int id)
        {
            var model = _service.Get(id);
            return new JsonResponse<Photo> { Success = model != null, Data = model };
        }

        // POST api/Photo
        public JsonResponse Post([FromBody]Photo value)
        {           
        ...
        // PUT api/Photo/5
        public JsonResponse Put([FromBody]Photo value)
        {           
