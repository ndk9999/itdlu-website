...
namespace DLUProject.API.Controllers
{
    ...
    {
        IServices<Category> _service;

        public CategoryController(IServices<Category> service)
        {
            this._service = service;
        }
        // GET api/account
        public JsonResponse<Category> Get()
        {
               // return new JsonResponse<string > { Success = true,  Data="Chao the gioi" };
              var items = _service.All();
              return new JsonResponse<Category> { Success = true, ListData = items };
        }

        // GET api/Category/5
        public JsonResponse<Category> Get(int id)
        {
            var model = _service.Get(id);
            return new JsonResponse<Category> { Success = model != null, Data = model };
        }

        // POST api/Category
        public JsonResponse Post([FromBody]Category value)
        {           
        ...
        // PUT api/Category/5
        public JsonResponse Put([FromBody]Category value)
        {           
