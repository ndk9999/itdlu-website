...
namespace DLUProject.API.Controllers
{
    ...
    {
        public IServices<Content> _service { get; set; }
        public ContentController(IServices<Content> service)
        {
            this._service = service;
        }
        // GET api/account
        public JsonResponse<Content> Get()
        {
            var items = _service.All();
            return new JsonResponse<Content> { Success = true, ListData = items };
        }

        // GET api/Content/5
        public JsonResponse<Content> Get(int id)
        {
            var model = _service.Get(id);
            return new JsonResponse<Content> { Success = model != null, Data = model };
        }

        // POST api/Content
        public JsonResponse Post([FromBody]Content value)
        {           
        ...
        // PUT api/Content/5
        public JsonResponse Put([FromBody]Content value)
        {           
