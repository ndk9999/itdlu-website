/* FileName: ResourceCategoryService.cs
Project Name: DLUProject
Date Created: 2/3/2015 8:54:12 AM
...
{
	/// <summary>
    /// Represents a ResourceCategoryService
    /// </summary>
    public class ResourceCategoryService : IServices<ResourceCategory>
    {
		private  IRepository<ResourceCategory> _objectProxy;      
        public ResourceCategoryService(IRepository<ResourceCategory> proxy)
        {
            this._objectProxy = proxy;
        }      
		public IQueryable<ResourceCategory> Table {  get { return _objectProxy.Table; }}
        
		 private string cacheKey = "ResourceCategory_Cache_Key";
        public List<ResourceCategory> All()
        {
            var cache = DataCache.GetCache(cacheKey) as List<ResourceCategory>;
            if (cache == null)
            {
                var items = _objectProxy.Table.Select(c => new ResourceCategory { 
...
MetaKeywords = c.MetaKeywords,
        ...
        public ResourceCategory Get(object id)
		{
			 return _objectProxy.Get(id);
			 // return All().SingleOrDefault(c=>c.WorkGroupID.Equals(id));
		}
		public ResourceCategory Get(Expression<Func<ResourceCategory, bool>> expression)
        {
            return _objectProxy.Get(expression);
        }
        public int Insert(ResourceCategory entity)
		{			 
		...
		public int Insert2(ResourceCategory entity)
		{		
			int kq =  _objectProxy.Insert2(entity);            			
			 DataCache.RemoveCache(cacheKey);
			 return kq;        			
		}		
		public int Insert(IEnumerable<ResourceCategory>items)
		{			
		...
		public int Update(ResourceCategory entity)
		{
		...
		public int Delete(ResourceCategory entity)
		{			
		...
		public int Delete(Expression<Func<ResourceCategory, bool>> expression)
		{
		...
		public int Delete(IEnumerable<ResourceCategory>items)
		{			
...
MetaKeywords = MetaKeywords.ToString(),
		...
		public string ExportToXml(List<ResourceCategory> items)
		{
			var sb = new StringBuilder();
            var stringWriter = new StringWriter(sb);
            var xmlWriter = new XmlTextWriter(stringWriter);
            xmlWriter.WriteStartDocument();
            xmlWriter.WriteStartElement("ResourceCategorys");
            xmlWriter.WriteAttributeString("Version", "1.0");

            foreach (var entity in items)
            {
                xmlWriter.WriteStartElement("ResourceCategory");
...
xmlWriter.WriteElementString("MetaKeywords", null, entity.MetaKeywords.ToString());
		...
		public void ExportToXlsx(Stream stream, List<ResourceCategory> items)
        {			
            if (stream == null)
            ...
            using (var xlPackage = new ExcelPackage(stream))
            {
                ...
                var worksheet = xlPackage.Workbook.Worksheets.Add("ResourceCategory");
