/* 
FileName: NoticeCategoryExtension.cs
...
{
	public class NoticeCategoryExtension
	{
		// YOUR CODE HERE
        IServices<NoticeCategory> _repository;
        public NoticeCategoryExtension(IServices<NoticeCategory> repo)
        {
            this._repository = repo;
        }

        public List<NoticeCategory> GetByParent(object id, bool isPublished)
        {
            if (isPublished)
        ...
        int level = -1;
        public List<NoticeCategory> GetByParent(List<NoticeCategory> list, int parentId, bool isPublished)
        {
            level++;
            string x = "";
            ...
            foreach (var d in GetByParent(parentId, isPublished))
            {
                ...
                NoticeCategory m = new NoticeCategory
                {
        ...
        public List<NoticeCategory> GetAllDropdownList(bool isPublished)
        {
            var myList = GetByParent(new List<NoticeCategory>(), 0, isPublished);
            myList.Insert(0, new NoticeCategory { CategoryID = 0, Breadcrumb = "---Chọn danh mục---" });
            return myList;
        }
        public List<NoticeCategory> GetAll(string queryString, bool isPublished)
        {
            var myList = GetByParent(new List<NoticeCategory>(), 0, isPublished);
        ...
        public PagedList<NoticeCategory> GetAllCategories(int pageIndex, int pageSize, string queryString, bool isPublished)
        {
            var myList = GetByParent(new List<NoticeCategory>(), 0, isPublished);
            if (!String.IsNullOrEmpty(queryString))
            {
        ...
        public string GetFormattedBreadCrumb(NoticeCategory category, string separator = ">>")
        {
            if (category == null)
        ...
        public string GetFormattedBreadCrumb(NoticeCategory category,
            List<NoticeCategory> allCategories,
            string separator = ">>")
        {
            if (category == null)
                throw new ArgumentNullException("category");
