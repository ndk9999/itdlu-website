/* FileName: DocCategoryService.cs
...
{
	/// <summary>
    /// Represents a DocCategoryService
    /// </summary>
    public class DocCategoryService : IServices<DocCategory>
    {
		private  IRepository<DocCategory> _objectProxy;      
        public DocCategoryService(IRepository<DocCategory> proxy)
        {
            this._objectProxy = proxy;
        }      
		public IQueryable<DocCategory> Table {  get { return _objectProxy.Table; }}
        
		 private string cacheKey = "DocCategory_Cache_Key";
        public List<DocCategory> All()
        {
            var cache = DataCache.GetCache(cacheKey) as List<DocCategory>;
            if (cache == null)
            {
                var items = _objectProxy.Table.Select(c => new DocCategory { 
...
MetaKeywords = c.MetaKeywords,
        ...
        public DocCategory Get(object id)
		{
			 return _objectProxy.Get(id);
			 // return All().SingleOrDefault(c=>c.WorkGroupID.Equals(id));
		}
		public DocCategory Get(Expression<Func<DocCategory, bool>> expression)
        {
            return _objectProxy.Get(expression);
        }
        public int Insert(DocCategory entity)
		{			 
		...
		public int Insert2(DocCategory entity)
		{		
			int kq =  _objectProxy.Insert2(entity);            			
			 DataCache.RemoveCache(cacheKey);
			 return kq;        			
		}		
		public int Insert(IEnumerable<DocCategory>items)
		{			
		...
		public int Update(DocCategory entity)
		{
		...
		public int Delete(DocCategory entity)
		{			
		...
		public int Delete(Expression<Func<DocCategory, bool>> expression)
		{
		...
		public int Delete(IEnumerable<DocCategory>items)
		{			
...
MetaKeywords = MetaKeywords.ToString(),
		...
		public string ExportToXml(List<DocCategory> items)
		{
			var sb = new StringBuilder();
            var stringWriter = new StringWriter(sb);
            var xmlWriter = new XmlTextWriter(stringWriter);
            xmlWriter.WriteStartDocument();
            xmlWriter.WriteStartElement("DocCategorys");
            xmlWriter.WriteAttributeString("Version", "1.0");

            foreach (var entity in items)
            {
                xmlWriter.WriteStartElement("DocCategory");
...
xmlWriter.WriteElementString("MetaKeywords", null, entity.MetaKeywords.ToString());
		...
		public void ExportToXlsx(Stream stream, List<DocCategory> items)
        {			
            if (stream == null)
            ...
            using (var xlPackage = new ExcelPackage(stream))
            {
                ...
                var worksheet = xlPackage.Workbook.Worksheets.Add("DocCategory");
