/* FileName: VideoCategoryService.cs
...
{
	/// <summary>
    /// Represents a VideoCategoryService
    /// </summary>
    public class VideoCategoryService : IServices<VideoCategory>
    {
		private  IRepository<VideoCategory> _objectProxy;      
        public VideoCategoryService(IRepository<VideoCategory> proxy)
        {
            this._objectProxy = proxy;
        }      
		public IQueryable<VideoCategory> Table {  get { return _objectProxy.Table; }}
        
		 private string cacheKey = "VideoCategory_Cache_Key";
        public List<VideoCategory> All()
        {
            var cache = DataCache.GetCache(cacheKey) as List<VideoCategory>;
            if (cache == null)
            {
                var items = _objectProxy.Table.Select(c => new VideoCategory { 
...
MetaKeywords = c.MetaKeywords,
        ...
        public VideoCategory Get(object id)
		{
			 return _objectProxy.Get(id);
			 // return All().SingleOrDefault(c=>c.WorkGroupID.Equals(id));
		}
		public VideoCategory Get(Expression<Func<VideoCategory, bool>> expression)
        {
		...
		public int Insert2(VideoCategory entity)
		{		
			int kq =  _objectProxy.Insert2(entity);            			
			 DataCache.RemoveCache(cacheKey);
			 return kq;        			
		}		
		public int Insert(IEnumerable<VideoCategory>items)
		{			
		...
		public int Delete(VideoCategory entity)
		{			
		...
		public int Delete(Expression<Func<VideoCategory, bool>> expression)
		{
		...
		public int Delete(IEnumerable<VideoCategory>items)
		{			
...
MetaKeywords = MetaKeywords.ToString(),
		...
		public string ExportToXml(List<VideoCategory> items)
		{
			var sb = new StringBuilder();
            var stringWriter = new StringWriter(sb);
            var xmlWriter = new XmlTextWriter(stringWriter);
            xmlWriter.WriteStartDocument();
            xmlWriter.WriteStartElement("VideoCategorys");
            xmlWriter.WriteAttributeString("Version", "1.0");

            foreach (var entity in items)
            {
                xmlWriter.WriteStartElement("VideoCategory");
...
xmlWriter.WriteElementString("MetaKeywords", null, entity.MetaKeywords.ToString());
		...
		public void ExportToXlsx(Stream stream, List<VideoCategory> items)
        {			
            if (stream == null)
            ...
            using (var xlPackage = new ExcelPackage(stream))
            {
                ...
                var worksheet = xlPackage.Workbook.Worksheets.Add("VideoCategory");
