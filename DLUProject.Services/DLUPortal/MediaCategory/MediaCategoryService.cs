/* FileName: MediaCategoryService.cs
...
{
	/// <summary>
    /// Represents a MediaCategoryService
    /// </summary>
    public class MediaCategoryService : IServices<MediaCategory>
    {
        private IRepository<MediaCategory> _objectProxy;
        public MediaCategoryService(IRepository<MediaCategory> proxy)
        {
            this._objectProxy = proxy;
        }
        public IQueryable<MediaCategory> Table { get { return _objectProxy.Table; } }
        
		 private string cacheKey = "MediaCategory_Cache_Key";
        public List<MediaCategory> All()
        {
            var cache = DataCache.GetCache(cacheKey) as List<MediaCategory>;
            if (cache == null)
            {
                var listX = _objectProxy.Table.Select(c => new MediaCategory
                {
                    ...
                    CountAlbums = c.MediaAlbums.Count
        ...
        public MediaCategory Get(object id)
		{
			 return _objectProxy.Get(id);
		}
		public MediaCategory Get(Expression<Func<MediaCategory, bool>> expression)
        {
            return _objectProxy.Get(expression);
        }
        public int Insert(MediaCategory entity)
		{			 
		...
		public int Insert2(MediaCategory entity)
		{		
			int kq =  _objectProxy.Insert2(entity);            			
			 DataCache.RemoveCache(cacheKey);
			 return kq;        			
		}		
		public int Insert(IEnumerable<MediaCategory>items)
		{			
		...
		public int Update(MediaCategory entity)
		{
		...
		public int Delete(MediaCategory entity)
		{			
		...
		public int Delete(Expression<Func<MediaCategory, bool>> expression)
		{
		...
		public int Delete(IEnumerable<MediaCategory>items)
		{			
...
MetaKeywords = MetaKeywords.ToString(),
		...
		public string ExportToXml(List<MediaCategory> items)
		{
			var sb = new StringBuilder();
            var stringWriter = new StringWriter(sb);
            var xmlWriter = new XmlTextWriter(stringWriter);
            xmlWriter.WriteStartDocument();
            xmlWriter.WriteStartElement("MediaCategorys");
            xmlWriter.WriteAttributeString("Version", "1.0");

            foreach (var entity in items)
            {
                xmlWriter.WriteStartElement("MediaCategory");
...
xmlWriter.WriteElementString("MetaKeywords", null, entity.MetaKeywords.ToString());
		...
		public void ExportToXlsx(Stream stream, List<MediaCategory> items)
        {			
            if (stream == null)
            ...
            using (var xlPackage = new ExcelPackage(stream))
            {
                ...
                var worksheet = xlPackage.Workbook.Worksheets.Add("MediaCategory");
