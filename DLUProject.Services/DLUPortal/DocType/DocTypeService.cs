/* FileName: DocTypeService.cs
Project Name: DLUProject
Date Created: 30/11/2014 2:35:00 PM
...
{
	/// <summary>
    /// Represents a DocTypeService
    /// </summary>
    public class DocTypeService : IServices<DocType>
    {
		private  IRepository<DocType> _objectProxy;      
        public DocTypeService(IRepository<DocType> proxy)
        {
            this._objectProxy = proxy;
        }      
		public IQueryable<DocType> Table {  get { return _objectProxy.Table; }}
        
		 private string cacheKey = "DocType_Cache_Key";
        public List<DocType> All()
        {
            var cache = DataCache.GetCache(cacheKey) as List<DocType>;
            if (cache == null)
            {
                var items = _objectProxy.Table.Select(c => new DocType { 
...
Description = c.Description,
        ...
        public DocType Get(object id)
		{
			 return _objectProxy.Get(id);
			 // return All().SingleOrDefault(c=>c.WorkGroupID.Equals(id));
		}
		public DocType Get(Expression<Func<DocType, bool>> expression)
        {
            return _objectProxy.Get(expression);
        }
        public int Insert(DocType entity)
		{			 
		...
		public int Insert2(DocType entity)
		{		
			int kq =  _objectProxy.Insert2(entity);            			
			 DataCache.RemoveCache(cacheKey);
			 return kq;        			
		}		
		public int Insert(IEnumerable<DocType>items)
		{			
		...
		public int Update(DocType entity)
		{
		...
		public int Delete(DocType entity)
		{			
		...
		public int Delete(Expression<Func<DocType, bool>> expression)
		{
		...
		public int Delete(IEnumerable<DocType>items)
		{			
...
Description = Description.ToString(),
		...
		public string ExportToXml(List<DocType> items)
		{
			var sb = new StringBuilder();
            var stringWriter = new StringWriter(sb);
            var xmlWriter = new XmlTextWriter(stringWriter);
            xmlWriter.WriteStartDocument();
            xmlWriter.WriteStartElement("DocTypes");
...
xmlWriter.WriteElementString("Description", null, entity.Description.ToString());
		...
		public void ExportToXlsx(Stream stream, List<DocType> items)
        {			
            if (stream == null)
            ...
            using (var xlPackage = new ExcelPackage(stream))
            {
                ...
                var worksheet = xlPackage.Workbook.Worksheets.Add("DocType");
