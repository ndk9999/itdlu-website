/* FileName: MediaService.cs
Project Name: DLUProject
Date Created: 27/11/2014 9:15:32 PM
Description: Auto-generated
Version: 1.0.0.0
Author:	Lê Thanh Tuấn - Khoa CNTT
Author Email: tuanitpro@gmail.com
Author Mobile: 0976060432
Author URI: http://tuanitpro.com
License: 

*/
using System;
using System.IO;
using System.Text;
using System.Drawing;
using System.Xml;
using OfficeOpenXml;
using System.Collections.Generic;
using System.Linq;
using System.Linq.Expressions;
using BLToolkit.Mapping;
using BLToolkit.DataAccess;
using BLToolkit.Data;
using BLToolkit.Data.Linq;
 

using DLUProject.Domain;
using DLUProject.Data;


using ColorLife.Core.Helper;
using OfficeOpenXml.Style;


namespace DLUProject.Services
{
	/// <summary>
    /// Represents a MediaService
    /// </summary>
    public class MediaService : IServices<Media>
    {
		private  IRepository<Media> _objectProxy;      
        public MediaService(IRepository<Media> proxy)
        {
            this._objectProxy = proxy;
        }      
		public IQueryable<Media> Table {  get { return _objectProxy.Table; }}
        
		 private string cacheKey = "Media_Cache_Key";
        public List<Media> All()
        {
            var cache = DataCache.GetCache(cacheKey) as List<Media>;
            if (cache == null)
            {
                var items = _objectProxy.Table.Select(c => new Media { 
					MediaID = c.MediaID,
AlbumID = c.AlbumID,
Alias = c.Alias,
Slug = c.Slug,
Name = c.Name,
MediaType = c.MediaType,
MimeType = c.MimeType,
Image = c.Image,
FileUrl = c.FileUrl,
Description = c.Description,
Caption = c.Caption,
AltTag = c.AltTag,
Copyright = c.Copyright,
SortOrder = c.SortOrder,
IsPublished = c.IsPublished,
CreatedByID = c.CreatedByID,
DateCreated = c.DateCreated,
ModifiedByID = c.ModifiedByID,
DateModified = c.DateModified,
DatePublished = c.DatePublished,
Hits = c.Hits,
IsDeleted = c.IsDeleted,

				}).ToList();
				
                DataCache.SetCache(cacheKey, items, DateTime.Now.AddDays(1));
                return items;
            }
            return cache;
        }
		 
        public Media Get(object id)
		{
			 return _objectProxy.Get(id);
			 // return All().SingleOrDefault(c=>c.WorkGroupID.Equals(id));
		}
		public Media Get(Expression<Func<Media, bool>> expression)
        {
            return _objectProxy.Get(expression);
        }
        public int Insert(Media entity)
		{			 
			
             int kq =  _objectProxy.Insert(entity);            			
			 DataCache.RemoveCache(cacheKey);
			 return kq;
		}	
		public int Insert2(Media entity)
		{		
			int kq =  _objectProxy.Insert2(entity);            			
			 DataCache.RemoveCache(cacheKey);
			 return kq;        			
		}		
		public int Insert(IEnumerable<Media>items)
		{			
			 
            int kq = _objectProxy.Insert(items);    
			DataCache.RemoveCache(cacheKey);
			 return kq;    			
		}
		public int Update(Media entity)
		{
			 
			int kq= _objectProxy.Update(entity);
			DataCache.RemoveCache(cacheKey);
			 return kq;    
		}
		public int Delete(Media entity)
		{			
			 
           int kq = _objectProxy.Delete(entity);
		   DataCache.RemoveCache(cacheKey);
			 return kq;    
		}
		public int Delete(Expression<Func<Media, bool>> expression)
		{
			 
			int kq= _objectProxy.Delete(expression);
			DataCache.RemoveCache(cacheKey);
			 return kq;    
		}
        public int Delete(object id)
		{
			 
			int kq= _objectProxy.Delete(id);
			DataCache.RemoveCache(cacheKey);
			 return kq;    
		}
		public int Delete(IEnumerable<Media>items)
		{			
			 
            int kq= _objectProxy.Delete(items);            
			DataCache.RemoveCache(cacheKey);
			 return kq;    
		}
        public int Delete()
		{			
			int kq = _objectProxy.Delete();
			DataCache.RemoveCache(cacheKey);
			 return kq;    
		}
		 
		#region ImportExport
		public void ImportFromXlsx(Stream stream)
        {
			using (var xlPackage = new ExcelPackage(stream))
            {
                // get the first worksheet in the workbook
                var worksheet = xlPackage.Workbook.Worksheets.FirstOrDefault();
                if (worksheet == null)
                    throw new ArgumentNullException("No worksheet found");

                //the columns
                var properties = new string[]
                {
                   "MediaID",
"AlbumID",
"Alias",
"Slug",
"Name",
"MediaType",
"MimeType",
"Image",
"FileUrl",
"Description",
"Caption",
"AltTag",
"Copyright",
"SortOrder",
"IsPublished",
"CreatedByID",
"DateCreated",
"ModifiedByID",
"DateModified",
"DatePublished",
"Hits",
"IsDeleted",

                };
                int iRow = 2;

                while (true)
                {
                    bool allColumnsAreEmpty = true;
                    for (var i = 1; i <= properties.Length; i++)
                        if (worksheet.Cells[iRow, i].Value != null && !String.IsNullOrEmpty(worksheet.Cells[iRow, i].Value.ToString()))
                        {
                            allColumnsAreEmpty = false;
                            break;
                        }
                    if (allColumnsAreEmpty)
                        break;
						
                    var MediaID = worksheet.Cells[iRow, GetColumnIndex(properties, "MediaID")].Value.ToInt();
var AlbumID = worksheet.Cells[iRow, GetColumnIndex(properties, "AlbumID")].Value.ToInt();
var Alias = worksheet.Cells[iRow, GetColumnIndex(properties, "Alias")].Value ?? string.Empty;
var Slug = worksheet.Cells[iRow, GetColumnIndex(properties, "Slug")].Value ?? string.Empty;
var Name = worksheet.Cells[iRow, GetColumnIndex(properties, "Name")].Value ?? string.Empty;
var MediaType = worksheet.Cells[iRow, GetColumnIndex(properties, "MediaType")].Value ?? string.Empty;
var MimeType = worksheet.Cells[iRow, GetColumnIndex(properties, "MimeType")].Value ?? string.Empty;
var Image = worksheet.Cells[iRow, GetColumnIndex(properties, "Image")].Value ?? string.Empty;
var FileUrl = worksheet.Cells[iRow, GetColumnIndex(properties, "FileUrl")].Value ?? string.Empty;
var Description = worksheet.Cells[iRow, GetColumnIndex(properties, "Description")].Value ?? string.Empty;
var Caption = worksheet.Cells[iRow, GetColumnIndex(properties, "Caption")].Value ?? string.Empty;
var AltTag = worksheet.Cells[iRow, GetColumnIndex(properties, "AltTag")].Value ?? string.Empty;
var Copyright = worksheet.Cells[iRow, GetColumnIndex(properties, "Copyright")].Value ?? string.Empty;
var SortOrder = worksheet.Cells[iRow, GetColumnIndex(properties, "SortOrder")].Value.ToInt();
var IsPublished = worksheet.Cells[iRow, GetColumnIndex(properties, "IsPublished")].Value.ToBool();
var CreatedByID = worksheet.Cells[iRow, GetColumnIndex(properties, "CreatedByID")].Value.ToInt();
var DateCreated = worksheet.Cells[iRow, GetColumnIndex(properties, "DateCreated")].Value.ToDateTime();
var ModifiedByID = worksheet.Cells[iRow, GetColumnIndex(properties, "ModifiedByID")].Value.ToInt();
var DateModified = worksheet.Cells[iRow, GetColumnIndex(properties, "DateModified")].Value.ToDateTime();
var DatePublished = worksheet.Cells[iRow, GetColumnIndex(properties, "DatePublished")].Value.ToDateTime();
var Hits = worksheet.Cells[iRow, GetColumnIndex(properties, "Hits")].Value.ToInt();
var IsDeleted = worksheet.Cells[iRow, GetColumnIndex(properties, "IsDeleted")].Value.ToBool();


                    var entity = new Media()
                    {
                        MediaID = MediaID,
AlbumID = AlbumID,
Alias = Alias.ToString(),
Slug = Slug.ToString(),
Name = Name.ToString(),
MediaType = MediaType.ToString(),
MimeType = MimeType.ToString(),
Image = Image.ToString(),
FileUrl = FileUrl.ToString(),
Description = Description.ToString(),
Caption = Caption.ToString(),
AltTag = AltTag.ToString(),
Copyright = Copyright.ToString(),
SortOrder = SortOrder,
IsPublished = IsPublished,
CreatedByID = CreatedByID,
DateCreated = DateCreated,
ModifiedByID = ModifiedByID,
DateModified = DateModified,
DatePublished = DatePublished,
Hits = Hits,
IsDeleted = IsDeleted,

                    };

                    _objectProxy.Insert(entity);
                    //next row
                    iRow++;
                }
            }
		}
		public string ExportToXml(List<Media> items)
		{
			var sb = new StringBuilder();
            var stringWriter = new StringWriter(sb);
            var xmlWriter = new XmlTextWriter(stringWriter);
            xmlWriter.WriteStartDocument();
            xmlWriter.WriteStartElement("Medias");
            xmlWriter.WriteAttributeString("Version", "1.0");

            foreach (var entity in items)
            {
                xmlWriter.WriteStartElement("Media");
				xmlWriter.WriteElementString("MediaID", null, entity.MediaID.ToString());
xmlWriter.WriteElementString("AlbumID", null, entity.AlbumID.ToString());
xmlWriter.WriteElementString("Alias", null, entity.Alias.ToString());
xmlWriter.WriteElementString("Slug", null, entity.Slug.ToString());
xmlWriter.WriteElementString("Name", null, entity.Name.ToString());
xmlWriter.WriteElementString("MediaType", null, entity.MediaType.ToString());
xmlWriter.WriteElementString("MimeType", null, entity.MimeType.ToString());
xmlWriter.WriteElementString("Image", null, entity.Image.ToString());
xmlWriter.WriteElementString("FileUrl", null, entity.FileUrl.ToString());
xmlWriter.WriteElementString("Description", null, entity.Description.ToString());
xmlWriter.WriteElementString("Caption", null, entity.Caption.ToString());
xmlWriter.WriteElementString("AltTag", null, entity.AltTag.ToString());
xmlWriter.WriteElementString("Copyright", null, entity.Copyright.ToString());
xmlWriter.WriteElementString("SortOrder", null, entity.SortOrder.ToString());
xmlWriter.WriteElementString("IsPublished", null, entity.IsPublished.ToString());
xmlWriter.WriteElementString("CreatedByID", null, entity.CreatedByID.ToString());
xmlWriter.WriteElementString("DateCreated", null, entity.DateCreated.ToString());
xmlWriter.WriteElementString("ModifiedByID", null, entity.ModifiedByID.ToString());
xmlWriter.WriteElementString("DateModified", null, entity.DateModified.ToString());
xmlWriter.WriteElementString("DatePublished", null, entity.DatePublished.ToString());
xmlWriter.WriteElementString("Hits", null, entity.Hits.ToString());
xmlWriter.WriteElementString("IsDeleted", null, entity.IsDeleted.ToString());

                xmlWriter.WriteEndElement();
            }
            xmlWriter.WriteEndElement();
            xmlWriter.WriteEndDocument();
            xmlWriter.Close();
            return stringWriter.ToString();
		}
		public void ExportToXlsx(Stream stream, List<Media> items)
        {			
            if (stream == null)
                throw new ArgumentNullException("stream");

            // ok, we can run the real code of the sample now
            using (var xlPackage = new ExcelPackage(stream))
            {
                // uncomment this line if you want the XML written out to the outputDir
                //xlPackage.DebugMode = true; 

                // get handle to the existing worksheet
                var worksheet = xlPackage.Workbook.Worksheets.Add("Media");
                //Create Headers and format them
                var properties = new string[]
                    {
                        "MediaID",
"AlbumID",
"Alias",
"Slug",
"Name",
"MediaType",
"MimeType",
"Image",
"FileUrl",
"Description",
"Caption",
"AltTag",
"Copyright",
"SortOrder",
"IsPublished",
"CreatedByID",
"DateCreated",
"ModifiedByID",
"DateModified",
"DatePublished",
"Hits",
"IsDeleted",

                    };
                for (int i = 0; i < properties.Length; i++)
                {
                    worksheet.Cells[1, i + 1].Value = properties[i];
                    worksheet.Cells[1, i + 1].Style.Fill.PatternType = ExcelFillStyle.Solid;
                    worksheet.Cells[1, i + 1].Style.Fill.BackgroundColor.SetColor(Color.FromArgb(184, 204, 228));
                    worksheet.Cells[1, i + 1].Style.Font.Bold = true;
                }
                int row = 2;
                foreach (var entity in items)
                {
                    int col = 1;
					worksheet.Cells[row, col].Value = entity.MediaID;
col++;
worksheet.Cells[row, col].Value = entity.AlbumID;
col++;
worksheet.Cells[row, col].Value = entity.Alias;
col++;
worksheet.Cells[row, col].Value = entity.Slug;
col++;
worksheet.Cells[row, col].Value = entity.Name;
col++;
worksheet.Cells[row, col].Value = entity.MediaType;
col++;
worksheet.Cells[row, col].Value = entity.MimeType;
col++;
worksheet.Cells[row, col].Value = entity.Image;
col++;
worksheet.Cells[row, col].Value = entity.FileUrl;
col++;
worksheet.Cells[row, col].Value = entity.Description;
col++;
worksheet.Cells[row, col].Value = entity.Caption;
col++;
worksheet.Cells[row, col].Value = entity.AltTag;
col++;
worksheet.Cells[row, col].Value = entity.Copyright;
col++;
worksheet.Cells[row, col].Value = entity.SortOrder;
col++;
worksheet.Cells[row, col].Value = entity.IsPublished;
col++;
worksheet.Cells[row, col].Value = entity.CreatedByID;
col++;
worksheet.Cells[row, col].Value = entity.DateCreated;
col++;
worksheet.Cells[row, col].Value = entity.ModifiedByID;
col++;
worksheet.Cells[row, col].Value = entity.DateModified;
col++;
worksheet.Cells[row, col].Value = entity.DatePublished;
col++;
worksheet.Cells[row, col].Value = entity.Hits;
col++;
worksheet.Cells[row, col].Value = entity.IsDeleted;
col++;

					 			

                    row++;
                }

                // save the new spreadsheet
                xlPackage.Save();
            }        
		}
		#endregion
		
		#region Utilities

        protected virtual int GetColumnIndex(string[] properties, string columnName)
        {
            if (properties == null)
                throw new ArgumentNullException("properties");

            if (columnName == null)
                throw new ArgumentNullException("columnName");

            for (int i = 0; i < properties.Length; i++)
                if (properties[i].Equals(columnName, StringComparison.InvariantCultureIgnoreCase))
                    return i + 1; //excel indexes start from 1
            return 0;
        }

        #endregion

		
    }
}

