...
namespace ColorLife.Core.Mvc
{
    internal class PagerBuilder
    {
        ...
        private const string CopyrightText = "\r\n<!--Dịch vụ thiết kế website chuyên nghiệp - Lê Thanh Tuấn -->\r\n";
        ...
        private string GenerateAnchor(PagerItem item)
        {
            ...
            if (_pagerOptions.UseJqueryAjax)
            {
                if (_html.ViewContext.UnobtrusiveJavaScriptEnabled)
                {
                    ...
                    string rs = string.IsNullOrEmpty(url) ? "<li class='active'><a href='#'>" + item.Text + "</a></li>" : "<li>" + tag.ToString(TagRenderMode.Normal) + "</li>";
        ...
        private MvcHtmlString GeneratePagerElement(PagerItem item)
        {
            ...
            if (item.Disabled) //first,last,next or previous page
                return CreateWrappedPagerElement(item, String.Format("<li class=\"active\"><a href='#'>{0}</a></li>", item.Text));
        ...
        private MvcHtmlString GenerateMsAjaxPagerElement(PagerItem item)
        {
            ...
            if (item.Disabled)
                return CreateWrappedPagerElement(item, string.Format("<li class=\"active\"><a href='#'>{0}123</a></li>", item.Text));
