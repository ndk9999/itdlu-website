...
namespace ColorLife.Core.Helper
{
    ...
    public partial class InstallerHelper
    {
        ...
        {
            return !String.IsNullOrEmpty(InstallerConfig.ConnectionString);
        }
        ...
        public static void RedirectToInstallationPage()
        {
            ...
            //    if (!thisPage.ToLower().Contains("install/default.]"))
            //    {
            ...
            //            {
            //                string installPath = CommonHelper.GetStoreLocation() + "install/default.aspx";
        ...
        public static string GetCurrentVersion(string connectionString)
        {
            ...
            try
            {
                string query = "SELECT * FROM [Setting] where [SettingName]='site.verision'";
                ...
                using (var conn = new SqlConnection(connectionString))
                {
                    ...
                    if (rdr.Read())
                    {
                        if (rdr["Settingvalue"] != null && rdr["Settingvalue"] != DBNull.Value)
                            version = rdr["Settingvalue"].ToString();
                    }
                }
            }
            catch
        ...
        public static string SetCurrentVersion(string connectionString, string version )
        {
            try
            {
                string query = string.Format("Update [Settings] SET [SettingValue]='{0}' WHERE [Settingname]='site.verision'", version);
        ...
        {
            return SaveConnectionString(name, connectionString, "System.Data.SqlClient");
        }
        ...
        public static void Backup(string fileName)
        {
            ...
            string sqlConnectionString = SqlDataHelper.GetConnectionString("ConnectionString");
