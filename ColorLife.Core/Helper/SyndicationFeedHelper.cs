...
{
    public class SyndicationFeedChannel
    {
    ...
    public class SyndicationFeedHelper
    {
        ...
        public SyndicationFeedHelper(SyndicationFeedChannel chanel, List<SyndicationFeedItem> items)
        {
            _rss = new XmlDocument();
            XmlDeclaration xmlDeclaration = _rss.CreateXmlDeclaration("1.0", "utf-8", null);
            _rss.InsertBefore(xmlDeclaration, _rss.DocumentElement);
            ...
            addRssChannel(_rss, chanel);
            foreach (var item in items)
            {
                addRssItem(_rss, item);
            }
        }
        private XmlDocument addRssChannel(XmlDocument xmlDocument, SyndicationFeedChannel channel)
        {
        ...
        private XmlDocument addRssItem(XmlDocument xmlDocument, SyndicationFeedItem item)
        {
        ...
        public void AddRssChannel(SyndicationFeedChannel channel)
        {
            _rss = addRssChannel(_rss, channel);
        }
        public void AddRssItem(SyndicationFeedItem item)
        {
