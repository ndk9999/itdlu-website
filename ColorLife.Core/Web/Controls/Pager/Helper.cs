    ...
    public partial class AspNetPager
    {
        ...
        private string GetHrefString(int pageIndex)
        {
            if (UrlPaging)
            {
                ...
                return BuildUrlString((urlPageIndex == -1) ? plcHolder : (urlPageIndex == 1 || (ReverseUrlPageIndex && urlPageIndex == PageCount) ? null : urlPageIndex.ToString())); //changed on 2012-11-11
            }
            ...
            if (!string.IsNullOrEmpty(queryString))
            {
                ...
                foreach (string pm in prms)
                {
                    ...
                    if (!string.IsNullOrEmpty(pName))
                    {
                        ...
                        {
                            ubuilder.Append(amp).Append(pName).Append("=").Append(nvArr[1]);
                            amp = "&amp;";
                        }
                    }
                }
