...
{
  public class ThumbnailHandler : BaseHttpHandler
  {
    #region Enumerations
    /// <summary>
    ...
    protected override void HandleRequest(HttpContext context)
    {
        ...
        {
            if (context.Request.QueryString[IMG_PARAM].Contains("http://"))
                return;
        ...
        else
        {
            if (context.Request.QueryString[IMG_PARAM].Contains("http://"))
            {
                ...
                using (Stream stream = webclient.OpenRead(context.Request.QueryString[IMG_PARAM]))
                {
                    Bitmap iconbitmap = new Bitmap(System.Drawing.Image.FromStream(stream));                    
                    iconbitmap.Save(context.Response.OutputStream, ImageFormat.Jpeg);
                }
            }
            else
            {
